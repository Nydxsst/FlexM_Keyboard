...
extern u8 NRF24L_Tx_Rx_Sta;

/* Num键短按 */
extern volatile u8 OnOff_Dectect_Key;
...
//	WS2812_Refresh();
	...
	NRF24L_Rx_Buf[3] = 'e';//4位的字符串
	NRF24L_Rx_Buf[4] = '\0';//4位的字符串
	
	NRF24L_Tx_Buf[0] = 'g';//4位的字符串
	NRF24L_Tx_Buf[1] = 'o';//4位的字符串
	NRF24L_Tx_Buf[2] = 'o';//4位的字符串
	NRF24L_Tx_Buf[3] = 'n';//4位的字符串
	...
	while(1)
	{
		...
		if(OnOff_Dectect_Key == 1)//按下Key1发送包
		{
			OnOff_Dectect_Key = 0;
